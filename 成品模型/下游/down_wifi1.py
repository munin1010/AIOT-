...
class MQTTController:
    def __init__(self):
        self.uart = machine.UART(1, tx=Pin(8),
                                 rx=Pin(9), baudrate=115200)
    ...
    def setup_wifi_mqtt(self, ssid,password,mqtt_server,
                        topic_sub,topic_pub1,ready_cmd):
    ...
    def process_response(self):
        if self.resp != b'':
            ...
            a = str(self.resp).replace('\\r\\n', ' ').\
                replace('b', ' ').replace("'", " ")
            c = a.split(' ')
            d=[i for i in c if i is not None and i !='']
            return d
